...
describe('requestJson', () => {
  ...
  it('reports a signal aborted with an AppError as its reason as a RequestAbortedError, not that error', async () => {
    const controller = new AbortController();
    controller.abort(new AppError('superseded'));
    mockFetch.mockRejectedValue('superseded');
  ...
  it('wraps a rejection that cannot be stringified instead of throwing', async () => {
    const reason = Object.create(null) as object;
    globalThis.fetch = vi.fn(() => Promise.reject(reason));

    const error = (await requestJson('/x'))._unsafeUnwrapErr();
