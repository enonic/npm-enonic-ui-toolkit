...
/**
 * A `Translate` over a phrase map, read at call time: whoever calls it after the phrases arrive
 * gets them. That alone does not re-render — through `@enonic/ui`'s `I18nProvider`, labels keep
 * their first wording until the function it was handed changes identity.
 */
...
export function comparePhrases(catalog: Phrases, bundle: Phrases): PhraseComparison {
  ...
  for (const [key, text] of Object.entries(catalog)) {
    const translated = Object.hasOwn(bundle, key) ? bundle[key] : undefined;
