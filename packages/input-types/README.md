...
| Piece                              | What it is                                                                                                                                                      |
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `FormRenderer`, `FormItemRenderer` | the form, one item; the sets render inside as occurrences to add, reorder, collapse and delete with a confirmation, an option set with its choice among options |
...
| `pruneUnselectedOptionData`        | an option set's data as it is saved                                                                                                                             |
...
| `InputTypeRegistry`, `inputTypeRegistry`, `createInputTypeRegistry`                       | the types a form can render, by name: the shared module-level value, not a window global, and a factory for a second one — a nested form's, a test's                                                                                                                       |
...
| the providers                                                                             | `ValidationVisibilityProvider`, `RawValueProvider`, `ServerErrorsProvider`, `LocaleProvider`, `FieldRegistryProvider`, `InputTypeRegistryProvider`, each with the hook that reads it                                                                                       |
| the hooks and helpers an input type is built from                                         | `useOccurrenceManager`, `usePropertyArray`, `useInputTypeDescriptor`, `computeDefaultValue`, `displayValue`, `getInputAccessibleName`, `handleMobileCompletionKeyDown`, `getLangAttributes` — what a type written outside the package reaches for, as the built-in ones do |
