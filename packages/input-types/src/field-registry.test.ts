...
import { FieldRegistry, type RevealOptions } from './field-registry';
...
type HandleMocks = {
  ...
  acquireProcessing: ReturnType<typeof vi.fn<(occurrenceId: string) => string | undefined>>;
...
function makeHandle(): HandleMocks {
  return {
    ...
    acquireProcessing: vi.fn<(occurrenceId: string) => string | undefined>(() => 'token-default'),
...
describe('FieldRegistry', () => {
  ...
  describe('acquireProcessing', () => {
    ...
    it('propagates handle rejection (already processing / stale ID) as null', () => {
      const handle = makeHandle();
      handle.acquireProcessing.mockReturnValueOnce(undefined);
      registry.register('.foo', handle);
