...
import { DateTimeDescriptor, truncateToMinutes } from './date-descriptors';
import type { DateTimeConfig } from './input-type-config';
...
describe('DateTimeDescriptor', () => {
  ...
  describe('readConfig', () => {
    ...
    it('ignores datetime with timezone (handled by separate input type)', () => {
      const config = DateTimeDescriptor.readConfig({
        default: [{ value: '2016-12-31T23:59+01:00' }],
      });
