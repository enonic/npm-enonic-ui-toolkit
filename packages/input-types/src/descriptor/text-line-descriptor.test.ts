...
describe('TextLineDescriptor', () => {
  ...
  describe('validate', () => {
    ...
    it('validates rawValue when stored value is null and maxLength is exceeded', () => {
      ...
      expect(results).toHaveLength(1);
      expect(results[0]!).toMatchObject({ key: 'enonic.inputTypes.validation.breaksMaxLength' });
    });
  ...
  describe('readConfig → validate integration', () => {
    ...
    it('treats maxLength 0 as unlimited (sentinel -1)', () => {
      const config = TextLineDescriptor.readConfig({ maxLength: [{ value: 0 }] });
      const results = TextLineDescriptor.validate(
        ValueTypes.STRING.newValue('a'.repeat(1000)),
        config,
      );
      expect(results).toEqual([]);
    });
