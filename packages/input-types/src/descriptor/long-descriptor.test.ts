...
import { LongDescriptor } from './number-descriptors';
...
describe('LongDescriptor', () => {
  ...
  describe('validate', () => {
    ...
    it('detects value below min', () => {
      ...
      expect(results[0]!).toMatchObject({ key: 'enonic.inputTypes.validation.breaksMin' });
    });
    ...
    it('detects value above max', () => {
      ...
      const results = LongDescriptor.validate(value, config);
      expect(results).toHaveLength(1);
      expect(results[0]!).toMatchObject({ key: 'enonic.inputTypes.validation.breaksMax' });
    });
    ...
    it('returns error when null value has non-empty rawValue (rejected input)', () => {
      ...
      expect(results).toHaveLength(1);
      expect(results[0]!).toMatchObject({ key: 'enonic.inputTypes.validation.invalid' });
    });
