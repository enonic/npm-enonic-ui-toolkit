...
import { InstantDescriptor } from './date-descriptors';
import type { InstantConfig } from './input-type-config';
...
describe('InstantDescriptor', () => {
  ...
  describe('readConfig', () => {
    ...
    it('parses absolute datetime with positive offset', () => {
      const config = InstantDescriptor.readConfig({
        default: [{ value: '2000-01-01T12:30+01:00' }],
      });
    ...
    it('parses absolute datetime with negative offset', () => {
      const config = InstantDescriptor.readConfig({
        default: [{ value: '2025-06-15T08:00-05:00' }],
      });
    ...
    it('parses absolute datetime with half-hour offset', () => {
      const config = InstantDescriptor.readConfig({
        default: [{ value: '2025-06-15T18:00+05:30' }],
      });
    ...
    it('parses absolute datetime with +00:00 offset', () => {
      const config = InstantDescriptor.readConfig({
        default: [{ value: '2025-06-15T14:30+00:00' }],
      });
    ...
    it('parses offset datetime with seconds', () => {
      const config = InstantDescriptor.readConfig({
        default: [{ value: '2025-06-15T14:30:45+02:00' }],
      });
    ...
    it('parses offset datetime with +04:00', () => {
      const config = InstantDescriptor.readConfig({
        default: [{ value: '2026-09-09T12:00:00+04:00' }],
      });
