...
import type { RadioButtonConfig } from './input-type-config';
import { RadioButtonDescriptor } from './option-descriptors';
...
describe('RadioButtonDescriptor', () => {
  ...
  describe('validate', () => {
    ...
    it('detects value not in allowed options', () => {
      ...
      expect(results[0]!).toMatchObject({ key: 'enonic.inputTypes.validation.notAnOption' });
    });
