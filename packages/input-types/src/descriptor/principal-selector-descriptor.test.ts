...
import { PrincipalSelectorDescriptor } from './principal-selector-descriptor';
...
describe('PrincipalSelectorDescriptor', () => {
  ...
  describe('readConfig', () => {
    it('parses principal types from config', () => {
      ...
      expect(config.principalTypes).toContain('user');
      expect(config.principalTypes).toContain('group');
    });
    ...
    it('filters out invalid principal type values', () => {
      ...
      expect(config.principalTypes[0]!).toBe('user');
    });
    ...
    it('parses ROLE type', () => {
      ...
      expect(config.principalTypes).toContain('role');
    });
