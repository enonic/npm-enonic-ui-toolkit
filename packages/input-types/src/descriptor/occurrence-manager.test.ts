...
import type { CheckboxConfig, NumberConfig, TextLineConfig } from './input-type-config';
import { DoubleDescriptor } from './number-descriptors';
import { OccurrenceManager } from './occurrence-manager';
...
function createManager(opts: { min?: number; max?: number; values?: string[] } = {}) {
  ...
  return new OccurrenceManager<TextLineConfig>(
    occurrences,
    TextLineDescriptor,
    config,
    initialValues,
...
describe('OccurrenceManager', () => {
  ...
  describe('validate', () => {
    ...
    it('detects maximum breach', () => {
      ...
      const mgr = new OccurrenceManager<TextLineConfig>(
        occurrences,
        TextLineDescriptor,
        config,
        values,
  ...
  describe('transient errors', () => {
    ...
    it('drops only the removed occurrence on remove(), preserving siblings via ID', () => {
      ...
      expect(state.occurrenceValidation[0]!.validationResults).toEqual([
        { message: 'A', transient: true },
      ]);
      expect(state.occurrenceValidation[1]!.validationResults).toEqual([
        { message: 'C', transient: true },
      ]);
    });
    ...
    it('follows the moved occurrence to its new position via stable ID', () => {
      ...
      const state = mgr.validate();
      expect(state.occurrenceValidation[2]!.validationResults).toEqual([
        { message: 'A', transient: true },
      ]);
      expect(state.occurrenceValidation[1]!.validationResults).toEqual([
        { message: 'C', transient: true },
    ...
    it('clearAllTransientErrors drops every entry', () => {
      ...
      expect(mgr.hasTransientErrors()).toBe(false);
      expect(
        mgr.validate().occurrenceValidation.every((ov) => ov.validationResults.length === 0),
      ).toBe(true);
    });
  ...
  describe('with DoubleDescriptor', () => {
    function createDoubleManager(opts: { min?: number; max?: number; values?: number[] } = {}) {
      const { min = 0, max = 0, values = [] } = opts;
      const occurrences = Occurrences.minmax(min, max);
      const config: NumberConfig = { min: undefined, max: undefined };
      const initialValues = values.map((v) => ValueTypes.DOUBLE.fromJsonValue(v));
      return new OccurrenceManager<NumberConfig>(
        occurrences,
        DoubleDescriptor,
        config,
        initialValues,
    ...
    it('detects maximum breach with double values', () => {
      ...
      const mgr = new OccurrenceManager<NumberConfig>(
        occurrences,
        DoubleDescriptor,
        config,
        values,
  ...
  describe('with CheckboxDescriptor', () => {
    function createCheckboxManager(opts: { min?: number; max?: number; values?: boolean[] } = {}) {
      ...
      return new OccurrenceManager<CheckboxConfig>(
        occurrences,
        CheckboxDescriptor,
        config,
        initialValues,
    ...
    it('validate always passes (no validation rules)', () => {
      ...
      const state = mgr.validate();
      expect(state.occurrenceValidation.every((ov) => ov.validationResults.length === 0)).toBe(
        true,
      );
    });
