...
/** Whether a server path is the field or anything under it, `field`, `field.x`, `field[2]`. */
export function matchesFieldPath(serverPath: string, fieldPath: string): boolean {
  return (
    serverPath === fieldPath ||
    serverPath.startsWith(`${fieldPath}.`) ||
    serverPath.startsWith(`${fieldPath}[`)
  );
}
...
/** The occurrence a server path addresses, 0 when it names none. */
...
/** The server's errors ahead of the descriptor's; an occurrence that no longer exists reports on the first. */
export function mergeServerErrors(
  ...
  return validation.map((entry, index) => {
    const extra = [
      ...(serverErrorsByOccurrence.get(index) ?? []),
      ...(index === 0 ? overflow : []),
    ];
    return extra.length > 0
      ? { ...entry, validationResults: [...extra, ...entry.validationResults] }
      : entry;
  });
}
