...
export type TagInputProps<C extends InputTypeConfig = InputTypeConfig> =
  SelfManagedComponentProps<C> & {
    /** Completions for the draft, debounced; the tag input itself knows no source of tags. */
...
type SortableDataEntry = {
  ...
  data?: { current?: { sortable?: SortableEntryData } };
};

type SortableDataEntryWithData = SortableDataEntry & {
  data: { current: { sortable: SortableEntryData } };
};
...
export function shouldRemoveLatestTag(
  key: string,
  draft: string,
  canRemove: boolean,
  hasModifier = false,
  ...
  return (
    !hasModifier && canRemove && draft.length === 0 && (key === 'Backspace' || key === 'Delete')
  );
}
...
function hasSortableData(
  entry: SortableDataEntry | null | undefined,
): entry is SortableDataEntryWithData {
  return entry?.data?.current?.sortable != null;
}

function getSortableEntries(
  currentEntry: SortableDataEntryWithData,
  droppableContainers: { getEnabled: () => Array<SortableDataEntry | undefined> },
): SortableDataEntryWithData[] {
  return droppableContainers
    .getEnabled()
    .filter(
      (entry): entry is SortableDataEntryWithData =>
        entry != null &&
        !entry.disabled &&
    ...
    .sort(
      (first, second) => first.data.current.sortable.index - second.data.current.sortable.index,
    );
}

/** Left and right move between tags; up and down do nothing, since the tags wrap as text does. */
const tagKeyboardCoordinates: KeyboardCoordinateGetter = (event, args) => {
  ...
  const currentEntry = (
    over != null ? droppableContainers.get(over.id) : droppableContainers.get(active?.id)
  ) as SortableDataEntry | undefined;
...
function getTagViewState(
  occurrences: Occurrences,
  enabled: boolean,
  valueCount: number,
): TagViewState {
  const isFixed =
    occurrences.getMinimum() > 0 && occurrences.getMinimum() === occurrences.getMaximum();
  return {
    canAdd: enabled && !occurrences.maximumReached(valueCount),
    canRemove: enabled && valueCount > 0,
...
function getNextFocusableElements(element: HTMLElement): HTMLElement[] {
  const container = element.closest(`[data-component="${TAG_INPUT_NAME}"]`);
  const focusableElements = Array.from(
    element.ownerDocument.querySelectorAll<HTMLElement>(FOCUSABLE_SELECTOR),
  )
...
function focusFirstAvailable(candidates: HTMLElement[]): boolean {
  for (const candidate of candidates) {
    if (!isTabTarget(candidate)) continue;
    candidate.focus();
...
/** A scroll during a drag ends it: the pointer and the lifted tag would otherwise drift apart. */
function useDragScrollListener(): DragScrollListener {
  const cleanupRef = useRef<(() => void) | null>(null);
  const clear = (): void => clearCleanupRef(cleanupRef);
  useEffect(() => () => clearCleanupRef(cleanupRef), []);
  const listen = (ownerDocument: Document, onScroll: () => void): void => {
    clear();
...
function compactHiddenTagSlots(
  values: readonly Value[],
  onMove: (fromIndex: number, toIndex: number) => void,
): void {
  let targetIndex = 0;
  values.forEach((value, index) => {
    if (!isRenderableTagValue(value)) return;
...
function getCompactedTagIndex(values: readonly Value[], index: number): number {
  let compactedIndex = 0;
  for (let currentIndex = 0; currentIndex < index; currentIndex += 1) {
    const value = values[currentIndex];
    if (value !== undefined && isRenderableTagValue(value)) {
      compactedIndex += 1;
    }
  }
    ...
    >
      <Input
        ...
        className={cn(
          ...
          invalid &&
            '[&>div[data-state]:focus-within]:border-error! [&>div[data-state]]:border-error!',
        )}
        value={draft}
...
const TagItem = ({
  ...
  const setRemoveButtonRef = (node: HTMLButtonElement | null): void => {
  ...
  let dragListeners: Omit<NonNullable<typeof listeners>, 'onKeyDown'> | undefined;
  if (listeners != null) {
  ...
  const handleDragButtonKeyDown: JSX.KeyboardEventHandler<HTMLButtonElement> = (event) => {
    ...
    if (event.key === 'ArrowRight' || event.key === 'ArrowDown') {
      ...
      event.currentTarget
        .closest('li')
        ?.querySelector<HTMLButtonElement>('[data-tag-label]')
        ?.focus();
      return;
    }
  ...
  const handleLabelButtonKeyDown: JSX.KeyboardEventHandler<HTMLButtonElement> = (event) => {
    ...
    if (event.key === 'ArrowLeft' || event.key === 'ArrowUp') {
      ...
      const dragButton = event.currentTarget
        .closest('li')
        ?.querySelector<HTMLButtonElement>('[data-tag-drag]');
      if (dragButton != null) {
        dragButton.focus();
      } else {
        onNavigate(-1);
  ...
  const handleRemoveButtonKeyDown: JSX.KeyboardEventHandler<HTMLButtonElement> = (event) => {
    ...
    if (!isRemoveButtonKey(event.key)) return;
    event.preventDefault();
    event.stopPropagation();
    onRemoveKey();
  };
    ...
    >
      ...
      {editing ? (
        <input
          ...
          data-tag-editor
          className={cn(
            'h-5 w-36 bg-transparent font-semibold outline-none',
            editInvalid && 'text-error',
          )}
          value={editDraft}
          disabled={!enabled}
...
/**
 * A list of string occurrences edited as chips: typed, pasted with separators, suggested,
 * edited in place, reordered. Empty occurrences the engine keeps for the minimum stay hidden
 * and are the first slots a new tag fills.
 */
export const TagInput = ({
  ...
  const suggestionsVisible =
    ...
    enabled &&
    canAdd &&
    hasRenderableTagLabel(normalizedDraft) &&
  ...
  const activeSuggestionId =
    activeSuggestion != null ? `${suggestionListId}-${activeSuggestionIndex}` : undefined;
  ...
  const firstVisibleFieldError = tagEntries
    .map((entry) => getFirstError(errors[entry.originalIndex]?.validationResults ?? [], t))
    .find(Boolean);
  const hiddenCustomError = hiddenErrors
    .flatMap((entry) => entry.validationResults.filter((result) => result.custom))
    .map((result) => resolveValidationMessage(result, t))
    .find(Boolean);
  ...
  const handleFieldActivate = (): void => {
  ...
  const focusTagAt = (index: number): void => {
  ...
  const focusLabelAt = (index: number): void => {
    requestAnimationFrame(() => {
      if (index < 0) return;
      if (index >= visibleTagCount) {
        inputRef.current?.focus();
      ...
      wrapperRef.current
        ?.querySelectorAll<HTMLButtonElement>('[data-tag-label]')
        .item(index)
        ?.focus();
    });
  };

  const focusRemoveAt = (index: number): void => {
    requestAnimationFrame(() => {
      if (index < 0) return;
      if (index >= visibleTagCount) {
        inputRef.current?.focus();
  ...
  const focusTagIndexNextFrame = (index: number): void => {
    requestAnimationFrame(() => {
      if (index < 0) return;
      tagRefs.current[index]?.focus();
    });
  ...
  const commitTagLabels = (
    ...
    const remainingCapacity =
      maximum === 0 ? Number.POSITIVE_INFINITY : Math.max(maximum - visibleTagCount, 0);
    ...
    rawLabels.forEach((rawLabel) => {
      if (labelsToCommit.length >= remainingCapacity) return;
      const normalizedLabel = normalizeTagDraft(rawLabel);
      if (!hasRenderableTagLabel(normalizedLabel) || knownLabels.has(normalizedLabel)) return;
    ...
    const nextFocusableElements =
      isMobile && focusTarget != null && !hasRoomForAnother
        ? getNextFocusableElements(focusTarget)
        : [];
    const usedHiddenSlots = Math.min(
      Math.max(values.length - visibleTagCount, 0),
      labelsToCommit.length,
    );
  ...
  const commitDraft = (
    ...
    if (hasTagLabel(values, normalized, excludedIndex) || !canAdd) {
      return { committed: false, usedHiddenSlot: false };
    }
    const { committedCount, usedHiddenSlots } = commitTagLabels([normalized], {
      focusTarget,
      clearDraft: true,
    });
    return { committed: committedCount > 0, usedHiddenSlot: usedHiddenSlots > 0 };
  };

  const commitSuggestion = (
    label: string,
    focusTarget: HTMLInputElement | null = inputRef.current,
  ): void => {
    commitTagLabels([label], { focusTarget: focusTarget ?? undefined, clearDraft: true });
    setSuggestions([]);
    setActiveSuggestionIndex(-1);
  };

  const prepareRemove = (): void => {
    skipBlurCommit.current = true;
  };

  const handleRemove = (index: number, options: RemoveTagOptions = {}): void => {
    setTouched(true);
  ...
  const handleTagNavigate = (index: number, direction: -1 | 1): void => {
    if (direction === -1) {
  ...
  const handleKeyboardDragMove = (index: number, direction: -1 | 1): void => {
    const targetIndex = index + direction;
    const from = tagEntries[index];
    const to = tagEntries[targetIndex];
    if (from === undefined || to === undefined) return;
    onMove(from.originalIndex, to.originalIndex);
    focusTagAt(targetIndex);
  };

  const startEditing = (id: string, label: string, selectText = true): void => {
    setEditingState({ id, selectText });
    setEditDraft(label);
    setIsInputActive(false);
  };

  const cancelEditing = (focusIndex?: number): void => {
  ...
  const commitEditing = (
    ...
    const nextFocusableElements =
      explicit && isMobile && !canAdd && focusTarget != null
        ? getNextFocusableElements(focusTarget)
        : [];
  ...
  const removeEditingAndEditPrevious = (): void => {
  ...
  const editLastTagFromInput = (draftInput: HTMLInputElement): void => {
  ...
  const getTagItemProps = (entry: TagEntry, index: number): TagItemProps => ({
    ...
    error: getFirstError(errors[entry.originalIndex]?.validationResults ?? [], t),
    enabled,
    ...
    onRemoveKey: () =>
      handleRemove(entry.originalIndex, { activateInput: true, commitCurrentDraft: true }),
    onRemove: () => handleRemove(entry.originalIndex, { commitCurrentDraft: true }),
  });

  const commitAndLeaveInput = (
    draftInput: HTMLInputElement,
    focusFn: (index: number) => void,
  ): void => {
    const { committed } = commitDraft(undefined, undefined, draftInput.value);
  ...
  const handlePaste: JSX.ClipboardEventHandler<HTMLInputElement> = (event) => {
    ...
    if (pastedLabels.length === 0) return;
    const currentDraft = normalizeTagDraft(draftRef.current);
    const labelsToCommit = hasRenderableTagLabel(currentDraft)
      ? [currentDraft, ...pastedLabels]
      : pastedLabels;
  ...
  const handleDragEnd = (event: DragEndEvent): void => {
    ...
    const from = tagEntries[ids.indexOf(String(active.id))];
    const to = tagEntries[ids.indexOf(String(over.id))];
    if (from === undefined || to === undefined) return;
    onMove(from.originalIndex, to.originalIndex);
  };
  ...
  const handleDragStart = (): void => {
    ...
    dragScrollListener.listen(ownerDocument, () => {
    ...
    <div data-component={TAG_INPUT_NAME} className="flex flex-col gap-y-2">
      <div
        ...
        className={cn(
          ...
          canAdd && 'cursor-text',
          hasErrors &&
            'border-error focus-within:border-error focus-within:ring-error hover:outline-error',
        )}
