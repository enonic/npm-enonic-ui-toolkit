...
function makeState(values: string[], min: number, max: number): OccurrenceManagerState {
  ...
  const manager = new OccurrenceManager<ComboBoxConfig>(
    occurrences,
    ComboBoxDescriptor,
    config,
    initialValues,
...
function makeDefaultArgs(
  ...
  min: number,
  max: number,
): SelfManagedComponentProps<ComboBoxConfig> {
  ...
  return {
    ...
    occurrences: Occurrences.minmax(min, max),
    config: makeConfig(),
...
const InteractiveDemo = (): ReactElement => {
  ...
  const managerRef = useRef(
    new OccurrenceManager<ComboBoxConfig>(occurrences, ComboBoxDescriptor, config, []),
  );
