...
import { FieldRegistryProvider } from '../context/field-registry-context';
...
function StatefulTextArea(
  props: InputTypeComponentProps<TextAreaConfig> & { initialValue?: Value },
) {
...
function HighlightDemo() {
  ...
  return (
    <div className="flex flex-col gap-y-3 p-4">
      ...
      <TextAreaInput
        {...defaultArgs}
        value={ValueTypes.STRING.newValue('Find me')}
        highlight={highlight}
      />
      <div className="h-96" />
    </div>
    ...
    <LocaleProvider locale="nb-NO">
      <div className="flex flex-col gap-y-3 p-4">
        <div className="max-w-120 text-sm text-subtle">
          ...
          <code>lang="nb"</code> and <code>spellcheck="true"</code> so the browser uses the
...
export const WithRtlLocale: Story = {
  ...
  render: () => (
    <LocaleProvider locale="ar-SA">
      <div className="flex flex-col gap-y-3 p-4">
        <div className="max-w-120 text-sm text-subtle">
          Wrapped in <code>LocaleProvider locale="ar-SA"</code>. The textarea renders right-to-left
          with <code>lang="ar" dir="rtl"</code>.
        </div>
        <TextAreaInput
          {...defaultArgs}
          value={ValueTypes.STRING.newValue('مرحبا بالعالم\nهذه منطقة نصية.')}
        />
      </div>
    </LocaleProvider>
  ),
...
function WithRegistryDemo() {
  ...
  useEffect(() => {
    const registration = registry.register(STORY_PATH, {
      ...
      acquireProcessing: (occurrenceId: string) => {
        if (occurrenceId !== OCCURRENCE_ID) return undefined;
        if (processingTokenRef.current.has(occurrenceId)) return undefined;
  ...
  return (
    <FieldRegistryProvider registry={registry}>
      <div className="flex flex-col gap-y-3 p-4">
        <div className="max-w-120 text-sm text-subtle">
          Acquire a processing lock via the registry. The textarea becomes read-only and shows a
          spinner. Release the lock to restore the normal state.
        </div>
        <TextAreaInput
          {...defaultArgs}
          value={ValueTypes.STRING.newValue(
  ...
  render: () => (
    <div className="w-80 space-y-6 p-4">
      ...
      <div>
        ...
        <TextAreaInput
          {...defaultArgs}
          value={ValueTypes.STRING.newValue('Cannot edit')}
          enabled={false}
        />
      </div>
      <div>
