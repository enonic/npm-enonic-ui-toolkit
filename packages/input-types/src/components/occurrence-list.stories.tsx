...
import { TextLineInput } from './text-line-input';
...
function TransientErrorsDemo({ min, max }: { min: number; max: number }): ReactElement {
  const occurrences = useMemo(() => Occurrences.minmax(min, max), [min, max]);
  const config = useMemo(makeConfig, []);
  const initialValues = useMemo(
    () => [ValueTypes.STRING.newValue('Alpha'), ValueTypes.STRING.newValue('Beta')],
    [],
  );
  ...
  return (
    <div className="flex flex-col gap-y-4 p-4">
      <div className="max-w-120 text-sm text-subtle">
        Click "Inject error" to push a transient error onto an occurrence — it shows immediately
        regardless of touched state. Edit the field to auto-clear it. Add/remove/move occurrences to
        see indices follow. Sync (the "Reset values" button below) clears all transient errors.
      </div>
  ...
  render: () => (
    <OccurrenceList
      ...
      input={makeInput(
        0,
        3,
        'Optional Multiple Empty (0:3)',
        '1 auto-filled null, no remove button',
      )}
      enabled={true}
    />
  ),
...
const InteractiveDemo = (): ReactElement => {
  const occurrences = useMemo(() => Occurrences.minmax(0, 5), []);
  const config = useMemo(() => makeConfig(), []);
...
const SingleStrategyDemo = (): ReactElement => {
  ...
  const serverOccurrences = useMemo(() => Occurrences.minmax(0, 0), []);
  const occurrences = useMemo(
    () => getEffectiveOccurrences('single', serverOccurrences),
    [serverOccurrences],
  );
    ...
    <div className="flex w-100 flex-col gap-y-4 p-4">
      <div className="w-96 rounded-sm bg-surface-primary p-3 text-sm">
        <p className="mb-2 font-medium">
          Checkbox with 0:0 server occurrences (normalized to 0:1):
        </p>
        <ul className="space-y-1 text-xs">
...
const InteractiveUnlimitedDemo = (): ReactElement => {
  const occurrences = useMemo(() => Occurrences.minmax(1, 0), []);
  const config = useMemo(() => makeConfig(), []);
  const input = useMemo(
    () => makeInput(1, 0, 'Unlimited (1:0)', 'At least one value required'),
    [],
  );
