...
import { InputField, InputFieldResolved } from './input-field';
...
const mocks = vi.hoisted(() => ({
  ...
  useRawValueMap: vi.fn((): RawValueMap | undefined => undefined),
...
vi.mock('../context/raw-value', () => ({
  useRawValueMap: mocks.useRawValueMap,
}));

vi.mock('../context/server-errors', () => ({
  useServerErrors: vi.fn(() => undefined),
}));

vi.mock('../context/field-registry-context', () => ({
...
vi.mock('../context/input-type-registry-context', () => ({
  useInputTypeRegistry: () => ({ getDefinition: mocks.getDefinition }),
}));

vi.mock('../hooks/use-property-array', () => ({
  usePropertyArray: mocks.usePropertyArray,
}));

vi.mock('../hooks/use-occurrence-manager', () => ({
  useOccurrenceManager: mocks.useOccurrenceManager,
}));

vi.mock('../utils/validation', () => ({
  getOccurrenceError: vi.fn(() => undefined),
...
function makeManagerState(value = ValueTypes.STRING.newValue('hello')) {
  return {
    ...
    occurrenceValidation: [
      { index: 0, breaksRequired: false, validationResults: [] as ValidationResult[] },
    ],
    totalValid: 1,
...
describe('InputField', () => {
  ...
  it('keeps the internal component dataPath object stable for the same field path', () => {
    ...
    expect(getChildAt(secondElement, 1).props.dataPath).toBe(
      getChildAt(firstElement, 1).props.dataPath,
    );
  });
  ...
  describe('AI affordances (handle contract)', () => {
    ...
    it('registers a FieldHandle exposing the new AI affordance methods', async () => {
      const { FieldRegistry } = await import('../field-registry');
      ...
      const { useFieldRegistry } = await import('../context/field-registry-context');
    ...
    it('blurs the focused input without emitting active-path null to subscribers', async () => {
      ...
      const blur = vi.fn();
      const mockEl = { blur } as unknown as HTMLElement;
