...
import type { SortableGridListProps } from './sortable-grid-list';
...
function makeProps(
  overrides: Partial<SortableGridListProps<Item>> = {},
): SortableGridListProps<Item> {
  return {
    items: ITEMS,
...
function renderList(overrides: Partial<SortableGridListProps<Item>> = {}): {
  list: VNode;
  rows: VNode[];
} {
  ...
  const rows = findAllElementsInTree(
    ...
    (element) =>
      ...
      typeof element.props.onfocusin === 'function' &&
      typeof element.props.onfocusout === 'function',
  );
...
function mountRows(rows: VNode[], descendantsByRow: FocusTargetNode[][] = []): RowNode[] {
  return rows.map((row, index) => {
    ...
    const node: RowNode = {
      contains: vi.fn(
        (target) => target === node || descendants.includes(target as FocusTargetNode),
      ),
      focus: vi.fn(() => {
        (row.props.onfocusin as ((event: unknown) => void) | undefined)?.({
          currentTarget: node,
          target: node,
        });
      }),
...
describe('SortableGridList', () => {
  beforeEach(() => {
    ...
    focusedTargetIndexSetter = vi.fn((value: number | ((current: number) => number)) => {
      focusedTargetIndexState =
        typeof value === 'function' ? value(focusedTargetIndexState) : value;
    });
    focusedItemIdState = ITEMS[0]?.id ?? null;
    focusedItemIdSetter = vi.fn(
    ...
    mocks.useState.mockImplementation((initial: unknown) => {
      ...
      if (
        resolved == null ||
        (typeof resolved === 'string' && ITEMS.some((item) => item.id === resolved))
      ) {
    ...
    mocks.useSortable.mockImplementation(({ id }: { id: string }) =>
      ensureSortableEntry(String(id)),
    );
  });
  ...
  it('syncs late-mounted descendants into the row roving model on focus', () => {
    ...
    (rows[0]!.props.onfocusin as ((event: unknown) => void) | undefined)?.({
    ...
    expect(lateButton.setAttribute).toHaveBeenCalledWith(
      'data-sortable-list-effective-tabindex',
      '0',
    );
  });
  ...
  it('does not restore focus after the list has blurred and the focused row is removed', () => {
    ...
    (list.props.onfocusin as ((event: unknown) => void) | undefined)?.({
      currentTarget: listNode,
      target: middleRowRemoveButton,
    });
    (list.props.onfocusout as ((event: unknown) => void) | undefined)?.({
      currentTarget: listNode,
