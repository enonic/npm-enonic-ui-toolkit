...
import type { SortableDragInfo, SortableListProps } from './sortable-list';
import { SortableList } from './sortable-list';
import { type DropProjection, projectTreeDrop } from './tree-projection';
      ...
      <div className="w-80">
        <SortableList
          ...
          onMove={(from, to) => {
            ...
            addLog(`Moved "${items[from]?.label ?? from}" from ${from} to ${to}`);
          }}
          enabled
          fullRowDraggable={fullRowDraggable}
          dragLabel="Drag to reorder"
...
function flattenTree(regions: TreeRegion[]): TreeRow[] {
  const rows: TreeRow[] = [
    { id: 'page', parentId: null, depth: 1, kind: 'item', label: 'Page', rowKind: 'page' },
  ];
...
function TreeDemo() {
  ...
  const isItemMovable = useCallback(
    (row: TreeRow) => row.kind === 'item' && row.rowKind !== 'page',
    [],
  );
  ...
  return (
    <div className="w-96">
      <p className="mb-3 text-subtle text-xs">
        Drag a row up or down — it walks through the valid drop slots. A region row's upper half
        inserts before it; its lower half enters the region at the start; moving past its lower edge
        selects the gap after it. A trailing item's lower half appends inside its region; moving
        past that item exits the region. Dragging down at the list edge steps out past the layout.
        Rows shift around a faded placeholder while the dragged clone re-indents to show its level.
        It dims when the drop is not allowed.
      </p>
      <SortableList
        items={rows}
        keyExtractor={keyExtractor}
        ...
        onMove={(_from, _to, info) => {
          if (info == null) return;
          const projection = project(info);
          const activeRow = rows[info.activeIndex];
          if (projection == null || !projection.allowed || activeRow === undefined) return;
          setRegions((prev) =>
            moveTreeItem(prev, activeRow.id, projection.containerId, projection.index),
          );
        }}
  ...
  render: () => (
    <Demo
      itemClassName={({ isDragging, isFocused }) =>
        cn(
          'gap-2 rounded-lg border border-transparent p-1',
          isFocused && !isDragging && 'border-ring/50',
        )
      }
    />
  ),
