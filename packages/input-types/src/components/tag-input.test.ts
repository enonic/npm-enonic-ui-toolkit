...
vi.mock('../i18n/use-phrases', () => ({
  useInputTypesPhrases: () => (key: string) => key,
}));

vi.mock('../context/validation-visibility', () => ({
...
function makeFocusableElement(
  ...
  const element: Record<string, any> = {
    ...
    closest: vi.fn((selector: string) =>
      selector === '[hidden], [inert]' && hiddenAncestor ? {} : null,
    ),
...
function getKeyboardSensorOptions(): Record<string, any> {
  const call = (
    mocks.useSensor.mock.calls as unknown as Array<[unknown, Record<string, any>]>
  ).find((entry) => entry[0] === 'KeyboardSensor');
  if (call == null) {
...
function getIconButtonProps(ariaLabel: string, index = 0): Record<string, any> {
  const props = mocks.iconButton.mock.calls
    .map((call) => call[0])
    .filter((entry) => entry['aria-label'] === ariaLabel)[index];
...
function getFirstDragButtonProps(): Record<string, any> {
  return getIconButtonProps('enonic.inputTypes.occurrence.reorder');
}

function getFirstRemoveButtonProps(): Record<string, any> {
  return getIconButtonProps('enonic.inputTypes.occurrence.remove');
}
...
function getSuggestionButton(
  label: string,
  overrides: Partial<TagInputProps> = {},
): Record<string, any> {
  const match = findElementInTree(
    getTagInputElement(overrides),
...
function mockTagEditingState({
  ...
  mocks.useState
    ...
    .mockImplementationOnce(() => [
      editingId == null ? null : { id: editingId, selectText },
      setEditingId,
    ])
...
describe('TagInput helpers', () => {
  ...
  it('returns minimum occurrence errors when no valid tags are present', () => {
    ...
    const message = getOccurrenceErrorMessage(
      Occurrences.minmax(1, 3),
      validation,
      mocks.useI18n(),
    );

    expect(message).toBe('enonic.inputTypes.occurrence.breaksMin:1');
  });
  ...
  it('suppresses occurrence errors when a tag already has a field error', () => {
    ...
    expect(
      getOccurrenceErrorMessage(Occurrences.minmax(1, 3), validation, mocks.useI18n()),
    ).toBeUndefined();
  });
  ...
  it('filters suggested labels already present in existing tags or the current draft', () => {
    ...
    expect(
      getSuggestedTagLabels(
        ['alpha', 'al', 'alpine', 'alpine', '  spaced  ', '   '],
        values,
        ' al ',
      ),
    ).toEqual(['alpine', 'spaced']);
  });
...
describe('TagInput', () => {
  ...
  it('focuses the mounted input after a completed field tap', () => {
    ...
    mocks.useRef
      ...
      .mockImplementationOnce(() => ({ current: { focus } }));
  ...
  it('keeps drag buttons out of the tab order before focus enters the component', () => {
    ...
    expect(getFirstDragButtonProps()['aria-label']).toBe('enonic.inputTypes.occurrence.reorder');
  });
  ...
  it('makes the last tag label the tab entry point when max tags hide the inline input', () => {
    ...
    expect(getIconButtonProps('enonic.inputTypes.occurrence.reorder', 0).tabIndex).toBe(-1);
    expect(getIconButtonProps('enonic.inputTypes.occurrence.reorder', 1).tabIndex).toBe(-1);
    expect(getIconButtonProps('enonic.inputTypes.occurrence.remove', 0).tabIndex).toBe(-1);
    expect(getIconButtonProps('enonic.inputTypes.occurrence.remove', 1).tabIndex).toBe(-1);
  ...
  it.each([
    ...
    expect(labelButton['aria-label']).toBe('enonic.inputTypes.action.edit: alpha');
  ...
  it('commits an edited tag with Enter', () => {
    ...
    editor.onKeyDown({
      ...
      preventDefault,
      stopPropagation,
      currentTarget: { value: 'renamed' },
    });
    editor.onfocusout({ currentTarget: { value: 'stale draft' } });
  ...
  it('gives the inline editor an accessible name', () => {
    ...
    ).toBe('enonic.inputTypes.action.edit: alpha');
  });
  ...
  it('does not suppress blur when editing starts again after Escape', () => {
    ...
    layoutEffects.at(-1)?.();
    editor.onfocusout({ currentTarget: { value: 'renamed' } });
  ...
  it('focuses the next field after mobile Enter edits a tag at the maximum', () => {
    ...
    const { element: disabledInput, focus: focusDisabled } = makeFocusableElement(ownerDocument, {
      disabled: true,
    });
    const { element: nextInput, focus: focusNext } = makeFocusableElement(ownerDocument);
    currentInput.value = 'renamed';
    ownerDocument.querySelectorAll = vi.fn(() => [
      currentInput,
      hiddenInput,
      disabledInput,
      nextInput,
  ...
  it('removes an empty edited tag and starts editing the previous tag on Backspace', () => {
    ...
    editor.onfocusout({ currentTarget: { value: '' } });
  ...
  it('disables sortable behavior when the field is disabled', () => {
    ...
    expect(mocks.useSortable).toHaveBeenNthCalledWith(
      1,
  ...
  it('delegates non-arrow keyboard drag movement to sortable keyboard coordinates', () => {
    ...
    const coordinateGetter = getKeyboardSensorOptions().coordinateGetter;
    const fallback = coordinateGetter(
      { code: 'Enter', preventDefault: vi.fn() } as unknown as KeyboardEvent,
      {
        active: undefined,
        currentCoordinates: { x: 0, y: 0 },
        context: {},
  ...
  it('adds a tag on Enter and clears the draft', () => {
    ...
    const focus = vi.fn();
    mocks.useState
      .mockImplementationOnce(() => ['alpha', setDraft])
      .mockImplementationOnce(() => [true, vi.fn()]);
  ...
  it.each([
    ...
    mocks.getIsMobile.mockReturnValue(true);
    mocks.useState
      .mockImplementationOnce(() => ['new', vi.fn()])
      .mockImplementationOnce(() => [true, vi.fn()]);
  ...
  it('compacts hidden tag slots before writing a new tag so the new tag appends after visible tags', () => {
    ...
    renderTagInput({
      ...
      errors: [
        makeOccurrenceValidation(0),
        makeHiddenBlankValidation(1),
        makeOccurrenceValidation(2),
      ],
      occurrences: Occurrences.minmax(0, 4),
    });
  ...
  it('filters async suggestions before showing them', async () => {
    ...
    const suggestTags = vi.fn(() =>
      Promise.resolve(['alpha', 'al', 'alpine', 'alpine', '  spaced  ', '   ']),
    );
    const setSuggestions = vi.fn();
    mocks.useEffect.mockImplementation((effect) => {
      effect();
  ...
  it('commits the draft and moves focus to the previous visible tag on Escape', () => {
    ...
    const tagRefs = {
      current: [{ focus: vi.fn() }, { focus: focusPreviousTag }, { focus: focusNewTag }],
    };
    const removeTagRefs = { current: [] };
  ...
  it('removes the original tag after committing a draft into a hidden slot by click', () => {
    ...
    getIconButtonProps('enonic.inputTypes.occurrence.remove', 1).onClick();
