...
import { OccurrenceList, type OccurrenceListRootProps } from './occurrence-list';
...
function getFieldErrorMessage(tree: unknown): unknown {
  const props = findFieldError(tree)?.props;
...
describe('OccurrenceList', () => {
  ...
  it('renders occurrence error when minimum is breached and no field errors', () => {
    ...
    expect(getFieldErrorMessage(tree)).toEqual({
      key: 'enonic.inputTypes.occurrence.breaksMin',
      values: [2],
    });
  });
  ...
  it('externalizes field errors for draggable occurrence rows and suppresses them while processing', () => {
    ...
    const state = makeState(
      [ValueTypes.STRING.newValue('a'), ValueTypes.STRING.newValue('b')],
      0,
      5,
    );
    const stateWithError: OccurrenceManagerState = {
      ...
      occurrenceValidation: state.occurrenceValidation.map((entry, index) =>
        index === 0
          ? { ...entry, validationResults: [{ message: 'Invalid value entered' }] }
          : entry,
      ),
    };
    ...
    const processingTree = OccurrenceList.Root(
      makeProps({
        ...
        processingOccurrenceIds: new Set(['occurrence-0']),
      }),
    );
